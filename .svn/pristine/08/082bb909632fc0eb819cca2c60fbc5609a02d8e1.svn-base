...
public class SuperBullet extends Animatable {
	...
	public SuperBullet(boolean flipped, int x, int y, Player parent) {
		loadAnims();
	...
	private void explode(){
		...
		if(GamePanel.allies.contains(parent) || GamePanel.player.equals(parent)){
			...
			for(int i=0; i<GamePanel.allies.size(); i++){
				...
				Rectangle pBounds = new Rectangle(px, py, 64, 64);
				if(pBounds.intersects(bounds)){
					GamePanel.allies.get(i).damage(DAMAGE);
				}
			}
